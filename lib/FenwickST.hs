...
type BIT s = VUM.STVector s Int

newBIT :: Int -> ST s (BIT s)
newBIT n = VUM.replicate (n + 1) 0
{-# INLINE newBIT #-}

buildBIT :: VU.Vector Int -> ST s (BIT s)
buildBIT vec = do
  let n = VU.length vec
...
-- | 1-indexed @[l, r)@
sumFromTo :: BIT s -> Int -> Int -> ST s Int
...
-- | 1-indexed
findMinIndexGT :: BIT s -> Int -> ST s Int
